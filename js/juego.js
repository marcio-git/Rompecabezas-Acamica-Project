...
const instrucciones = ["Mueve las piezas.", "Usar las flechas del teclado."];
...
function mostrarInstrucciones(instrucciones) {
  ...
  for (let i = 0; i < instrucciones.length; i++) {
    ...
    mostrarInstruccionEnLista(inst,id);
  };
    //COMPLETO
}

/* COMPLETO: Crear función que agregue la última dirección al arreglo de movimientos
...
function chequearSiGano() {
  for (let i = 0; i < grilla.length; i++) {
    for (let j = 0; j < grilla[i].length; j++) {
      let grillaActual = grilla[i][j];
      if (grillaActual !== grillaDesordenada[i][j]) {
        return false;
      }
    }
  }
